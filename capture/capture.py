...
import threading
import os
from capture.detection.detect import DetectionStore
...
class CaptureThread(threading.Thread):
        ...
        try:
            while not self.stop_event.is_set():
                if (time.time() * 1000) - self.current_clip_start > (self.clip_length * 1000):
                    self.current_clip_start = time.time() * 1000
                    self.latest_clip_id = self.check_and_update_clip()
                
                request = self.camera.capture_request()
                try:
                    with metrics.time("capture"):
    ...
    def check_and_update_clip(self):
        latest_detection = self.detection_store.latest()
        has_detection = latest_detection is not None and len(latest_detection.boxes) > 0 # type: ignore

        if self.recording and self.latest_clip_id is not None:
            self.camera.stop_encoder()
            self.storage_thread.end_clip(self.latest_clip_id, ended_at=time.time() * 1000)
            self.recording = False
